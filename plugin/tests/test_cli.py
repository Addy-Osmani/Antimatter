...
from antimatter_bridge.cli import init_plugin, main
...
@patch('antimatter_bridge.cli.shutil.copy2')
@patch('antimatter_bridge.cli.Path.mkdir')
@patch('antimatter_bridge.cli.Path.exists')
def test_init_plugin_creates_directories(mock_exists, mock_mkdir, mock_copy):
    ...
    with patch('antimatter_bridge.cli.Path') as MockPath:
