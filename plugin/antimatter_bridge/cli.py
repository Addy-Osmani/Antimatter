...
def init_plugin():
    ...
    try:
        ...
        # 2. Set up the daemon config directory
        daemon_dir = home_dir / ".antimatter_daemon"
        if not daemon_dir.exists():
            print(f"Creating daemon config directory: {daemon_dir}")
            daemon_dir.mkdir(parents=True, exist_ok=True)
            
        config_file = daemon_dir / "config.json"
        if not config_file.exists():
            print(f"Copying default config.json to {config_file}...")
            shutil.copy2(assets_dir / "config.json.example", config_file)
        else:
            print(f"Config file already exists at {config_file}, skipping.")
        
        # 3. Create skills directory and copy SKILL.md
...
def start_server():
    ...
    print("Starting Antimatter Bridge Server...")
    try:
        server.main()
