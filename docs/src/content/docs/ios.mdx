---
title: iOS App
description: Complete reference for the Antimatter iOS app — SwiftUI, SwiftTerm, CryptoKit, Face ID, and native PTY terminal.
sidebar:
  order: 4
---
...
The Antimatter iOS app is a native companion application built with **SwiftUI** and **Swift 6**, providing the same full-featured experience as the Android app — live agent monitoring, remote prompting, workspace browsing, and a native PTY terminal — all secured by Ed25519 cryptography and end-to-end encryption.
...
└── TerminalCoordinator      # Native PTY terminal
```
...
```swift title="TerminalViewController.swift (simplified)"
import SwiftTerm
