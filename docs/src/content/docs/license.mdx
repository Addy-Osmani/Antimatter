...
Because the mobile apps include advanced terminal emulation modules (`terminal-emulator` and `terminal-view`) originally derived from [Termux](https://termux.dev/), the entirety of the mobile application source code and compiled binaries are distributed under the GPLv3 to strictly comply with upstream requirements. 
