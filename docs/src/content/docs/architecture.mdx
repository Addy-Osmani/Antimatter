...
The Gateway spawns `cloudflared` to expose a secure `wss://` endpoint — no firewall rules, no port forwarding, no static IP required. TryCloudflare generates a free random URL; a custom domain via Cloudflare Zero Trust provides the strongest security.
