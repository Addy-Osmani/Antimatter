...
On Linux/macOS you can run `antimatter-gateway start --daemon` to run it as a background process. Use `antimatter-gateway status` to check it's alive.
:::
...
No. TryCloudflare works with zero configuration. A domain (Cloudflare Zero Trust) is recommended for the strongest security.
